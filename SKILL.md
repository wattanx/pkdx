...
以下を**並列で**Readする:
- `$POKEDEX_DIR/type/type.json` → タイプ相性表（`type` 配列の中で `scarlet_violet` を含むエントリを使用）
...
取得したポケモン名で以下を実行:
```bash
bash $SKILL_DIR/scripts/query_pokemon.sh "<ポケモン名>" "<version>"
```
...
```bash
bash $SKILL_DIR/scripts/query_moves.sh "<globalNo>" "<version>"
```
...
攻めの穴を埋めるタイプで候補を検索:
```bash
bash $SKILL_DIR/scripts/search_by_type.sh "<穴を埋めるタイプ>" "" "0" "<version>"
```
...
弱点を受けられるタイプで候補を検索:
```bash
bash $SKILL_DIR/scripts/search_by_type.sh "<弱点を受けるタイプ>" "" "0" "<version>"
```
...
```bash
bash $SKILL_DIR/scripts/search_by_type.sh "<補完タイプ>" "" "100" "<version>"
```
...
各仮想敵のデータ取得:
```bash
bash $SKILL_DIR/scripts/query_pokemon.sh "<仮想敵名>" "<version>"
```
...
| 状況 | 対応 |
|------|------|
| pokedex.db が見つからない | Phase 0でセットアップ手順を案内し終了 |
| query_pokemon.sh の結果が空 | ポケモン名の確認を再度依頼。リージョンフォーム・メガフォームの指定が必要な可能性を案内 |
| query_moves.sh の結果が空 | version値の不一致の可能性。waza系テーブルのversion値はMixed Case（Scarlet_Violet等）であることを考慮 |
| Smogon Stats取得失敗 | フォールバック順で試行。全失敗時は手動入力モードに切り替え |
| type.jsonの読み込み失敗 | pokedexリポジトリの確認を依頼 |
