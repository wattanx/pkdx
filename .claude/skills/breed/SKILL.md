...
| # | 質問 | header | オプション | multiSelect |
|---|------|--------|-----------|-------------|
| 1 | 入力方式を選んでください | 入力方式 | 対話で構築する(desc: 従来通りの Phase 1-7), ゲームのスクショから取り込む(desc: 能力+ステータス画面の2枚を OCR) | false |

- `対話で構築する` → 下記「Phase 1 対話モード」へ
- `ゲームのスクショから取り込む` → [Phase 1-Vision](#phase-1-vision-champions-スクショ取り込み) へ
...
## Phase 1-Vision: Champions スクショ取り込み

Champions のゲーム画面 (「能力」画面 + 「ステータス」画面) から 1 体分の情報を抽出し、breed cache を冪等に構築する。成功後は **Phase 8 (保存)** に直接ジャンプし、Phase 2-7 を skip する。

### 1. スクショパス入力

**AskUserQuestion**（2問を一括）:

| # | 質問 | header | オプション | multiSelect |
|---|------|--------|-----------|-------------|
| 1 | 「能力」画面スクショのファイルパスを入力してください (.png / .jpg) | 能力画面 | Other (desc: 絶対パス) | false |
| 2 | 「ステータス」画面スクショのファイルパスを入力してください | ステータス画面 | Other (desc: 絶対パス) | false |

### 2. Vision 抽出

Read ツールで 2 枚の画像を順に読み、以下を vision で抽出する:

- **ポケモン名** (日本語カタカナ)、性別マーク (♂/♀/無性別)
- **性格名** が画面にあれば直接読む (例: 「ようき」)。なければ各ステータスの ↑/↓ マーカー列を記録
- **特性**、**持ち物**
- **SP 配分** (6 ステータス、Champions 限定、合計 ≤ 66、各 ≤ 32)
- **実数値** (6 ステータス、検算用)
- **技 4 つ** (名前のみ)
...
```bash
$PKDX query "<name>" --version champions --format json
```

種族値・タイプ・特性候補を取得。Champions 側で未登録なら `--version scarlet_violet` で fallback。それでも空ならユーザーに名前修正を依頼。

### 4. SP 逆算検証

```bash
$PKDX stat-reverse "<name>" --stats "<HP>,<A>,<B>,<C>,<D>,<S>" --version champions --format json
```

出力 SP と vision 抽出 SP を比較。複数解が返る場合は「いずれかと一致」で OK。不一致なら ⚠ を表示しユーザーに抽出値の修正を促す。

### 5. 性格確定

優先順位:
1. ↑↓マーカーが読めていれば → 性格テーブル (上記) から直接特定
2. 読めない場合 → SP + 実数値の整合性で候補を絞る (`team-builder/references/champions_sp.md` の性格逆算手順を参照)
3. 候補 1 つ → 自動確定 / 複数候補 → AskUserQuestion で候補のみ提示

**全 25 性格の列挙は禁止** (UX 負荷が大きい)。

### 6. 技詳細補完

```bash
$PKDX moves "<name>" --version champions --format json
```

抽出した 4 つの技名を DB 照合し、`type/category/power/accuracy` を埋める。未登録技があればユーザー確認で修正。

### 7. 抽出結果表示と確認

```
=== スクショ抽出結果 ===
名前: マンムー (♀)  globalNo: 0473
性格: ようき (S↑ C↓)   特性: あついしぼう   持ち物: こだわりスカーフ
SP: H20 A32 B0 C0 D0 S14 (合計 66)
実数値: H205 A182 B100 C81 D80 S145  [stat-reverse 一致 ✓]
技: じしん / つららおとし / つららばり / ばかぢから
```

メガ石 (「○○ナイト」) を持ち物に検出したら下記を追記:

```
⚠ メガ石持ちが検出されました。
   メガ進化の戦闘評価は task B (メガ進化 state 拡張) 実装後に有効になります。
   現状は通常フォームとして扱います。
```
...
| # | 質問 | header | オプション | multiSelect |
|---|------|--------|-----------|-------------|
| 1 | この抽出でよいですか? | 確認 | はい(desc: cache 組み立てに進む), 修正する(desc: 項目を個別に修正), やり直す(desc: スクショパスから再入力) | false |

- `修正する` → どの項目を修正するか AskUserQuestion で選び、個別に上書き
- `やり直す` → 手順 1 から再実行
...
```bash
$PKDX init-cache pokemon > "$CACHE_FILE"
```

2. 抽出結果を cache JSON にマージ (pokemon / build セクション、`version: "champions"`, `regulation: "M-A"` 明記)

3. 既存の .meta.json を glob → `pkdx import-check` で冪等性判定:

```bash
POKE_NAME="<抽出した日本語名>"
EXISTING_DIR="$REPO_ROOT/box/pokemons/$POKE_NAME"

# 既存 .meta.json を {path, content} 形式で収集
...
jq -n \
  ...
  '{kind: "pokemon", cache: $cache[0], existing: $existing}' | \
  $PKDX import-check
```
...
- `{"status":"new"}` → そのまま Phase 8 へ

4. ユーザー承認後、**Phase 8 (保存)** に直接ジャンプ。Phase 8 で `pkdx write pokemon` が md + .meta.json を並行出力する (冪等性の次回判定に使用)
